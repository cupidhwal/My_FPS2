...
namespace Unity.FPS.Gameplay
{
    public class PlayerInputHandler : MonoBehaviour
    {
        ...
        public int GetSwitchWeaponInput()
        {
            if (CanProcessInput())
            {
                if (Input.GetAxis(GameConstants.k_AxisNameNextWeapon) > 0) return 1;
                else if (Input.GetAxis(GameConstants.k_AxisNameNextWeapon) < 0) return -1;
                else if (Input.GetAxis(GameConstants.k_MouseAxisNameScrollWheel) > 0) return 1;
                else if (Input.GetAxis(GameConstants.k_MouseAxisNameScrollWheel) < 0) return -1;
            }
